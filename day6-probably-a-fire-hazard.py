...
code='''turn on 887,9 through 959,629
...
turn off 102,229 through 674,529'''.split('\n')

n_code=[]
for i in xrange(0,len(code)):
    code[i]=code[i].split(' ')
    if len(code[i])==4:
        n_code.append([0,code[i][1].split(','),code[i][3].split(',')])
    else:
        if code[i][1]=='on':
            n_code.append([1,code[i][2].split(','),code[i][4].split(',')])
        else:
            n_code.append([2,code[i][2].split(','),code[i][4].split(',')])
...
def on(x1,y1,x2,y2,m):
    for i in xrange(int(x1),int(x2)+1):
        for j in xrange(int(y1),int(y2)+1):
            mat[i][j]=1

def off(x1,y1,x2,y2,m):
    for i in xrange(int(x1),int(x2)+1):
        for j in xrange(int(y1),int(y2)+1):
            mat[i][j]=0

def togg(x1,y1,x2,y2,m):
    for i in xrange(int(x1),int(x2)+1):
        for j in xrange(int(y1),int(y2)+1):
            if mat[i][j]==0:
                mat[i][j]=1
...
for i in xrange(0,len(n_code)):
    if n_code[i][0]==0:
        togg(n_code[i][1][0],n_code[i][1][1],n_code[i][2][0],n_code[i][2][1],mat)
    elif n_code[i][0]==1:
        on(n_code[i][1][0],n_code[i][1][1],n_code[i][2][0],n_code[i][2][1],mat)
    elif n_code[i][0]==2:
        off(n_code[i][1][0],n_code[i][1][1],n_code[i][2][0],n_code[i][2][1],mat)
